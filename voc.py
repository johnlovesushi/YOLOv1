...
class VOCDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # difficult = torch.as_tensor(
        #     [int(detection['difficult']) for detection in label['annotation']['object']])

        labels_tensor = torch.as_tensor(
            [self.classes[detection['name']] for detection in label['annotation']['object']])
        bboxes_xyxy_tensor = torch.as_tensor([[float(detection['bndbox']['xmin']),
                                               float(
                                                   detection['bndbox']['ymin']),
                                               float(
                                                   detection['bndbox']['xmax']),
                                               float(detection['bndbox']['ymax'])] for detection in label['annotation']['object']], dtype=float)

        w, h = float(label['annotation']['size']['width']), float(
            label['annotation']['size']['height'])
        bboxes_xywh_tensor = utils.x1y1x2y2_convert_to_xywh(bboxes_xyxy_tensor)
        scaled_bboxes_xywh_tensor = bboxes_xywh_tensor / \
            torch.tensor([w, h, w, h], dtype=torch.float32).expand_as(
                bboxes_xyxy_tensor)
        # scaled_bboxes_xyxy_tensor = bboxes_xyxy_tensor / \
        #     torch.tensor([w, h, w, h], dtype=torch.float32).expand_as(
        #         bboxes_xyxy_tensor)
        if self.transform_scale_translate == True:
            image, transform_vals = scale_image(image)
            scaled_bboxes_xywh_tensor = scale_translate_bounding_box(
                scaled_bboxes_xywh_tensor, transform_vals)

        if self.transform:
            data, scaled_bboxes_xywh_tensor = self.transform(
                image, scaled_bboxes_xywh_tensor)
    ...
    def _encode(self, scaled_bboxes, label):
        ...
        for i in range(num_of_boxes):
            ...
            if target[coord_y, coord_x,  20] == 0:
                ...
                target[coord_y, coord_x,  21:25] = torch.tensor(
